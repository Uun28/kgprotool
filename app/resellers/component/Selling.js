...
export default function AdminSelling() {
  ...
  useEffect(() => {
    setLoading(true);
    fetch(`${API2}?mode=getpenjualan`, { credentials: 'include' })
      .then(res => res.json())
      .then(data => {
        setSales(data.data || []);
        setLoading(false);
    ...
    <div className="max-w-5xl mx-auto w-full py-8 px-2 sm:px-4">
              ...
              paginated.map((s, idx) => (
                <tr key={s.id} className="group border-b border-blue-50 last:border-b-0 hover:bg-blue-50/40 transition">
                  ...
                  <td className="px-4 py-4 align-middle text-left">
                    ...
                    `}>
                      {s.licensed?.replace('month', ' Months')}
                    </span>
                  </td>
                  {/* Date */}
      ...
      <div className="sm:hidden flex flex-col gap-4">
        {loading ? (
          ...
          paginated.map(s => (
            <div key={s.id} className="rounded-2xl border border-blue-100 bg-white/90 shadow px-4 py-3 flex flex-col gap-2 hover:-translate-y-1 hover:shadow-xl transition">
